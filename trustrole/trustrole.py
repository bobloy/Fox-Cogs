...
class TrustRole:
    ...
    async def trustset_trust(self, ctx, role: discord.Role = None, threshold: int = None):
        ...
        if threshold is None or threshold <= 0:
            await self.bot.say(
                "Must provide a threshold >= 0, see `{}help trustset trust` for details".format(ctx.prefix))
            return
    ...
    async def trustset_distrust(self, ctx, role: discord.Role = None, threshold: int = None):
        ...
        if role is None:
            ...
            await self.bot.say(
                "Distrusting is now disabled\nRun `{}help trustset distrust` for details".format(ctx.prefix))
            dataIO.save_json(self.file_path, self.data)
            return

        if threshold is None or threshold <= 0:
            await self.bot.say(
                "Must provide a threshold >= 0, see `{}help trustset distrust` for details".format(ctx.prefix))
            return
